...
const confirmPurchase = (req) => {
    ...
    cartItems.forEach(cartItem => {
        items.forEach(item => {
            if (cartItem.id === item.id) item.numInStock = item.numInStock - cartItem.quantity
        })
    })
...
const getSearchResults = (req) => {
    ...
    let getSearchResults = items.filter(item => {if (item.name.toLowerCase().includes(userInput.toLowerCase())) return item});
    ...
    items.filter(item => {if (item.category.toLowerCase().includes(userInput.toLowerCase())) getSearchResults.push(item)});
...
const getCompanyProducts = (req) => {
    ...
    companies.forEach(company => {if (company.name === companyName) companyId = company.id});

    // filters through the items to find those released by that specific company
    let filteredProducts = items.filter(item => {if (item.companyId === companyId) return item});
...
const sortCategory = (req) => {
    ...
    let filteredItems = items.filter(item => {if (item.category === category) return item});
